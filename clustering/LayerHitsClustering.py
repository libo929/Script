...
for event in reader:
	...
	for iHit in range(0, nHit) :
		...
		hitPos[layer-1].append( TVector3(pos[0], pos[1], pos[2]) )
	...
	for iLayer in range(0, 50):
		nHit = len(hitPos[iLayer])
		print 'layer ', iLayer + 1, ', # of hit: ', nHit

	fig = plt.figure()
	ax = fig.add_subplot(111, projection='3d')

	colors = cycle('bgrcmykbgrcmykbgrcmykbgrcmyk')
	#for k, col in zip(hitPos[20], colors):
	#	plt.plot(k[0], k[1], col + '.')
	#for k in hitPos[9]:
	#	plt.plot(k[0], k[1], 'ro')
